...
class HeartSignal:
    def __init__(self, curve="heart", title="Love U", frame_num=20, seed_points_num=2000, seed_num=None,
                 highlight_rate=0.3,
                 background_img_dir="", set_bg_imgs=False, bg_img_scale=0.2, bg_weight=0.3, curve_weight=0.7,
                 frame_width=1080, frame_height=960, scale=10.1,
                 base_color=None, highlight_points_color_1=None, highlight_points_color_2=None, wait=100, n_star=5,
                 m_star=2):
        super().__init__()
        self.curve = curve
        self.title = title
        ...
        if os.path.exists(background_img_dir) and len(img_paths) > 0 and set_bg_imgs:
            ...
            for img in self.bg_imgs[1:]:
                ...
                if width_close:
                    ...
                    if img.shape[0] > first_bg.shape[0]:
                        ...
                        img = np.concatenate([np.zeros([fill_top, width, 3]), img, np.zeros([fill_bottom, width, 3])],
                                             axis=0)
                else:
            ...
            assert all(img.shape[0] == first_bg.shape[0] and img.shape[1] == first_bg.shape[1] for img in
                       self.bg_imgs), "背景图片宽和高不一致"
        ...
        for i in range(times):
            for frame in frames:
                ...
                if len(self.bg_imgs) > 0 and self.set_bg_imgs:
                    frame = cv2.addWeighted(self.bg_imgs[i % len(self.bg_imgs)], self.bg_weight, frame,
                                            self.curve_weight, 0)
